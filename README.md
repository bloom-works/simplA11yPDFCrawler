...
| Document       | `TaggedTest`            | Checks whether the document has a PDF structure tree and is explicitly marked as tagged. PDFs that fail this check may still have a structure tree that can be inspected by other checks.                                                                                                                 |
...
| Annotations    | `TaggedAnnotationsTest` | Checks link annotations and page-owned widget annotations against the structure tree using `/StructParent`, `/ParentTree` and matching `/OBJR` references. Top-level AcroForm widget fields are reported under `TaggedFormFieldsTest` instead. This is a structural approximation.                        |
...
| `StructTreeRootPresent`            | Whether the PDF has a `/StructTreeRoot`. Structure-based checks can still run when this is true, even if `TaggedTest` fails.                                                                   |
| `MarkedAsTagged`                   | Whether the PDF is explicitly marked as tagged in `/MarkInfo`.                                                                                                                                 |
...
| `TaggedAnnotationsTest`            | Whether link annotations and page-owned widget annotations are connected to the structure tree through `/StructParent`, `/ParentTree` and a matching `/OBJR` reference.                        |
| `TaggedAnnotationIssues`           | Details for checked annotations that are missing `/StructParent`, do not resolve through `/ParentTree`, map to an unexpected structure type, or lack a matching `/OBJR`.                       |
...
simpla11ypdf/
  ...
  widget_annotations.py   # Shared helpers for classifying form/page-owned widget annotations
